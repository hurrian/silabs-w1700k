...
import { FirmwareNames, ApplicationNames, ApplicationType, ApplicationTypeToFirmwareType, mdiFirmware, LegacyTypeToFirmwareType, } from './const';
...
let FlashingDialog = class FlashingDialog extends LitElement {
    ...
    generateProbeMethods(PyApplicationType) {
        const baudrates = new Map();
        const probe_methods = [];
        const valueToKeyMap = Object.entries(ApplicationType).reduce((acc, [key, value]) => {
            acc[value] = key;
            return acc;
        }, {});
        const baudKeys = Object.keys(this.manifest.baudrates);
    ...
    async onPyodideLoaded() {
        ...
        const { baudrates, probe_methods } = this.generateProbeMethods(PyApplicationType);
